...
public class LineRendererInspector : Editor 
{
	...
	void OnEnable() 
	{
		...
		LineRenderer first = objects[0] as LineRenderer;
		...
		//Cast
		renderer = new LineRenderer[objects.Length];
		...
		{
			//Cast
			renderer[i] = objects[i] as LineRenderer;
	...
	public override void OnInspectorGUI ()
	{
		...
		if( EditorGUI.EndChangeCheck() ) {
			foreach(LineRenderer r in renderer )
			{
